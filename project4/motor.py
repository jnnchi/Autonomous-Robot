...
class Motor():
    def __init__(self, enableL, backwardR, forwardL, enableR, backwardL, forwardR):
    ...
    def move_forward(self, distance):
        ...
        # 0.7 sec determined amount of time needed to move forward by the length of the robot
        time.sleep(0.49 * distance + 0.7)
        self.stop()

    def move_backward(self, distance):
        # setting output (go backward but not forward)
        GPIO.output(self.__forwardL, False)
        GPIO.output(self.__backwardL, True)
        GPIO.output(self.__forwardR, False)
        GPIO.output(self.__backwardR, True)

        self.__enableL_pwm.start(20 * 0.6)
        self.__enableR_pwm.start(20)

        # 1 sec is the determined amount of time needed to move backward by the length of the robot
        time.sleep(0.55 * distance + 1)
        self.stop()

    def turn_left(self, angle):
        # setting output (right wheels go forward, left wheels go backward)
        GPIO.output(self.__forwardL, False)
        GPIO.output(self.__backwardL, True)
        GPIO.output(self.__forwardR, True)
        GPIO.output(self.__backwardR, False)

        self.__enableL_pwm.start(20)
        self.__enableR_pwm.start(25)

        time.sleep(0.0077 * angle)
        self.stop()

    def turn_right(self, angle):
        # setting output (left wheels go forward, right wheels go backward)
        GPIO.output(self.__forwardL, True)
        GPIO.output(self.__backwardL, False)
        GPIO.output(self.__forwardR, False)
        GPIO.output(self.__backwardR, True)
